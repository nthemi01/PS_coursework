...
double noise::cubic_func(double x, const double param0, const double param1, const double param2, const double param3){
...
std::vector<std::function<double(double)>> noise::cubic_spline_functions(std::vector<double>& rdlist, const double step){
  ...
  *ypp.rbegin() = 0;
  ...
  for(int i = 1; i < size; ++i){
    ...
    y_prime[i] = (ypp[i]-diag_down[i]*y_prime[i-1]) / (diag_main[i]-diag_down[i]*up_prime[i-1]);
  }
  y_prime[size] = (ypp[size]-diag_down[size]*y_prime[size-1]) / (diag_main[size]-diag_down[size]*up_prime[size-1]);
  ...
  std::generate(splines.begin(),splines.end(),[&step,&index,&mval,&rdlist]{
    double param[4] = {
      rdlist[index],
    ...
    auto cubic = std::bind(noise::cubic_func, _1, param[0],param[1],param[2],param[3]);
    index++;
    return cubic;
  });
  return splines;
...
std::vector<double> noise::coherent_series(std::vector<std::function<double(double)>> splines_function, int samples, const double step){
  
  const int size = splines_function.size();
  std::vector<double> result(size*samples);
  for(int i = 0; i < size*samples; ++i){
    result[i] = splines_function[int(i/samples)]((i%samples)*step/samples);
  }
...
std::vector<std::vector<double>> noise::coherent_map(const int size1, const int size2, const int sample1, const int sample2){

  std::vector<std::vector<double>> semi_coherent_noise_2d(size2+1,std::vector<double>(size1*sample1+1));
  ...
  // transpose the map
  std::vector<std::vector<double>> semi_coherent_noise_2dT(size1*sample1+1,std::vector<double>(size2+1));
  ...
  for(int i = 0; i < size1*sample1+1; ++i){
    auto dummy_coherent_noise = noise::coherent_series(noise::cubic_spline_functions(semi_coherent_noise_2dT[i]),sample2);
