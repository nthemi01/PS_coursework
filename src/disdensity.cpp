...
cimg_library::CImg<unsigned int>  output::get_img( std::vector<std::vector<double>> density,
                                                   bool savefig, 
...
cimg_library::CImg<unsigned int> output::get_img( std::vector<std::vector<double>> density,
    ...
    for ( ; pixel != line1->end() && terrain != line2->end(); ++pixel, ++terrain) {
        unsigned int pixel_R, pixel_G, pixel_B;
        if (*terrain) {
            double red   = -3 * (*pixel - 0.75) * (*pixel - 0.75) + 1;
            double green = -3 * (*pixel - 0.25)* (*pixel - 0.25)  + 1;
...
void output::screen::assign( cimg_library::CImg<unsigned int> img_left,  
                             cimg_library::CImg<unsigned int> img_right) {
